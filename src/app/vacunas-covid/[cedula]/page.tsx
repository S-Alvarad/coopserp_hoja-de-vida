...
import { VacunasCovidForm } from "@/components/vacunas-covid/vacunas-covid-form"

interface PageProps {
   params: { cedula: string };
}

export default async function PostulanteConyuge({ params }: PageProps) {
   ...
   return (
      <div className="bg-muted relative flex min-h-svh flex-col items-center justify-center gap-6 lg:p-6 md:p-10">
         ...
         <div className="flex w-full md:max-w-2xl lg:max-w-3xl xl:max-w-4xl flex-col gap-6 mx-auto lg:p-2">
            <VacunasCovidForm  cedula={cedula} />
         </div>
      </div>
   )
}
